...
type TalentDomain struct {
	DomainName   string
	Weekday      int
	MaterialName string
}

func (talentDomain *TalentDomain) QueryTalentDomainsHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// 动态构建SQL和参数
		baseSQL := "SELECT domain_name, weekday, material_name FROM talent_domains WHERE 1=1"
...
// 查询所有天赋本
func (talentDomain *TalentDomain) QueryAllDomains() ([]string, error) {

	rows, err := config.DB.Query(`SELECT DISTINCT domain_name FROM talent_domains ORDER BY domain_name`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// 查询所有天赋书
func (talentDomain *TalentDomain) QueryAllTalents() ([]string, error) {
	rows, err := config.DB.Query(`SELECT DISTINCT material_name FROM talent_domains ORDER BY material_name`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
